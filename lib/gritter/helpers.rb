    ...
    def gflash *args
      ...
      if session[:gflash].present?
        ...
        session[:gflash].each do |key, value|
          #Rails.logger.debug "key = #{key}, value = #{value.inspect}"
          value.each do |gflash_value|
            #Rails.logger.debug "gflash_value = #{gflash_value}"

            gritter_options = { :image => key, :title => titles[key], :nodom_wrap => nodom_wrap }
            if gflash_value.is_a?(Hash)
              vals = gflash_value.symbolize_keys
              text = vals.has_key?(:value) ? (vals[:value] and vals.delete(:value)) : nil
              gritter_options.merge!(vals)
            else
              text = gflash_value
            end
            flashes.push(add_gritter(text, gritter_options))
            #Rails.logger.debug "(4) >>>>>> #{flashes.inspect}"
          end
        end
        session[:gflash] = nil
